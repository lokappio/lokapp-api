...
describe("Tests OWNER", () => {
    ...
    describe("Test invitation", () => {
        ...
        it("Creation", async () => {
            ...
            await EdgeHelper.validRequest(InvitationHelper.createInvitation(app, tester.id, dto));
        });
        ...
        it("Delete", async () => {
            ...
            await EdgeHelper.validRequest(InvitationHelper.deleteInvitation(app, tester.id, project.id, invitation.id));
        });
    });
    ...
    describe("Test project", () => {
        ...
        it("Delete", async () => {
            await EdgeHelper.validRequest(ProjectHelper.deleteProject(app, tester.id, project.id)); 
        });
    });
    ...
    describe("Test user", () => {
        ...
        it("Update Manager into Owner", async () => {
            const dto = new UpdateRoleDto({
                role: Role.Owner
            });
            await EdgeHelper.validRequest(ProjectHelper.updateRoleOfUser(app, tester.id, project.id, admin.id, dto));
        });

        it("Update Manager into Translator", async () => {
            const dto = new UpdateRoleDto({
        ...
        it("Update Translator into Owner", async () => {
            let relation = await userProjectRepository.findOne({where: {
                user: admin,
                project: project
            }});
        ...
        it("Update Translator into Manager", async () => {
            let relation = await userProjectRepository.findOne({where: {
                user: admin,
                project: project
            }});
        ...
        it("Delete", async () => {
            await EdgeHelper.validRequest(ProjectHelper.removeUserFromProject(app, tester.id, project.id, admin.id));
        });
    });
});