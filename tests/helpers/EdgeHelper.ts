...
export default class EdgeHelper {
    ...
    public static async requestWithoutUserID(request: request.Test): Promise<void> {
        const answer = await request;
        expect(answer.status).toBe(404);
    }

    public static async requestWithInvalidUserID(request: request.Test): Promise<void> {
        const answer = await request;
        expect(answer.status).toBe(404);
    }
    ...
    public static async unauthorized(request: request.Test): Promise<void> {
        const answer = await request;
        expect(answer.status).toBe(401);
    }

    public static async validRequest(request: any): Promise<void> {
        const answer = await request;
        expect(answer.status).toBeGreaterThanOrEqual(200);
        expect(answer.status).toBeLessThan(300);
    }

    public static async invalidRequest(request: any): Promise<void> {
        this.roleGuardError(request);
    }
}